...
app.config['MYSQL_PASSWORD'] = os.environ.get(
    'MYSQL_PASSWORD', 'zq]1SX(yMIW-2Wcv')
...
def home():
    try:
        ...
        cursor.execute(
            'SELECT * FROM payment_methods WHERE user_id = %s', (session['id'],))
        ...
        cursor.execute(
            'SELECT * FROM user_accounts WHERE userid = %s', (session['id'],))
        ...
        total_balance = sum(
            float(method['balance']) for method in payment_methods) if payment_methods else 0
        ...
        total_expenses = float(
            result['total_expenses']) if result['total_expenses'] else 0

        total_income = total_balance + total_expenses

        # Get budget limit with parameterized query
        cursor.execute(
            'SELECT limits FROM limitss WHERE user_id = %s ORDER BY id DESC LIMIT 1', (session['id'],))
        limit_result = cursor.fetchone()
        budget_limit = float(
            limit_result['limits']) if limit_result and limit_result['limits'] else 0
...
def update_settings():
    try:
        currency = sanitize_input(request.form.get('currency', 'GHS'))
        ...
        # Here you would typically update the database as well
        cursor = mysql.connection.cursor()
        cursor.execute('UPDATE register SET currency = %s WHERE id = %s',
                       (currency, session['id']))
        mysql.connection.commit()
        cursor.close()
...
def adding():
    try:
        ...
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute(
            'SELECT id, account_name, account_type, balance FROM user_accounts WHERE userid = %s', (session['id'],))
        user_accounts = cursor.fetchall()
        cursor.close()
...
def addexpense():
    if request.method == 'POST':
        # Expecting datetime-local format
        datetime_str = request.form.get('date', '')
        expensename = sanitize_input(
            request.form.get('expensename', '').strip())
        ...
        try:
            ...
            flash(
                "Invalid date/time format! Please select a valid date and time.", "error")
        ...
        if not (one_week_ago <= expense_date <= today):
            flash(
                f"Date must be within the last 7 days (from {one_week_ago} to {today})", "error")
            return redirect(url_for('adding'))
        ...
        try:
            ...
            cursor.execute(
                ...
                (session['id'], expense_date, expensename,
                 amount, paymode, category, account_id)
            )
            mysql.connection.commit()
            cursor.close()

            # Make sure you store email in session on login
...
def display():
    try:
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute(
            'SELECT * FROM expenses WHERE userid = %s ORDER BY date DESC', (session['id'],))
...
def delete(expense_id):
    try:
        ...
        cursor.execute(
            'DELETE FROM expenses WHERE id = %s AND userid = %s', (expense_id, session['id']))
        ...
        if cursor.rowcount > 0:
            ...
            flash(
                'Expense not found or you do not have permission to delete it.', 'error')
...
def edit(expense_id):
    try:
        ...
        cursor.execute(
            'SELECT * FROM expenses WHERE id = %s AND userid = %s', (expense_id, session['id']))
        row = cursor.fetchone()
        cursor.close()
        ...
        try:
            ...
            cursor.execute(
                ...
                (date, expensename, amount, paymode,
                 category, expense_id, session['id'])
            )
            ...
            if cursor.rowcount > 0:
                ...
                flash(
                    'Expense not found or you do not have permission to update it.', 'error')
...
def get_chart_data():
    try:
        ...
        if period == 'week':
            ...
            expense_dict = {result['expense_date'].strftime(
                '%d %b'): float(result['total']) for result in results}
            ...
            results = cursor.fetchall()
            expense_dict = {result['week_number']: float(
            ...
            results = cursor.fetchall()
            expense_dict = {result['month_number']: float(
                result['total']) for result in results}
...
def limitnum():
    if request.method == "POST":
        ...
        try:
            cursor = mysql.connection.cursor()
            cursor.execute(
                'INSERT INTO limitss (user_id, limits) VALUES (%s, %s)', (session['id'], number))
            mysql.connection.commit()
            cursor.close()
...
def today():
    try:
        ...
        cursor.execute(
            'SELECT TIME(date), amount FROM expenses WHERE userid = %s AND DATE(date) = DATE(NOW())', (session['id'],))
        texpense = cursor.fetchall()

        # Get all today's expenses
        cursor.execute(
            'SELECT * FROM expenses WHERE userid = %s AND DATE(date) = DATE(NOW()) ORDER BY date DESC', (session['id'],))
        expense = cursor.fetchall()
...
def month():
    try:
        ...
        cursor.execute(
            'SELECT DATE(date), SUM(amount) FROM expenses WHERE userid = %s AND MONTH(DATE(date)) = MONTH(now()) GROUP BY DATE(date) ORDER BY DATE(date)', (session['id'],))
        texpense = cursor.fetchall()

        # Get all this month's expenses
        cursor.execute(
            'SELECT * FROM expenses WHERE userid = %s AND MONTH(DATE(date)) = MONTH(now()) ORDER BY date DESC', (session['id'],))
        expense = cursor.fetchall()
...
def year():
    try:
        ...
        cursor.execute(
            'SELECT MONTH(date), SUM(amount) FROM expenses WHERE userid = %s AND YEAR(DATE(date)) = YEAR(now()) GROUP BY MONTH(date) ORDER BY MONTH(date)', (session['id'],))
        texpense = cursor.fetchall()

        # Get all this year's expenses
        cursor.execute(
            'SELECT * FROM expenses WHERE userid = %s AND YEAR(DATE(date)) = YEAR(now()) ORDER BY date DESC', (session['id'],))
        expense = cursor.fetchall()
